...
import { getCompanyInfo, updateCompanyInfo, getStatistics, updateStatistic } from '../services/api';
...
const Settings = () => {
  ...
  const handleAddStat = () => {
    if (newStat.label && newStat.value) {
      setStatistics((prev) => [...prev, { ...newStat, id: Date.now() }]);
            ...
            <List>
              {statistics.map((stat, index) => (
                <ListItem key={stat.id || index} divider>
                  ...
                  <ListItemSecondaryAction><IconButton color="error" onClick={() => handleDeleteStat(stat.id)}><Delete /></IconButton></ListItemSecondaryAction>
                </ListItem>
              ))}
            </List>
